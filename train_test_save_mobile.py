...
checkpoint = ModelCheckpoint(f"MobileNet_checkpoint_best_{{val_loss:.4f}}.h5", 
                             monitor='val_loss', 
                             save_best_only=True, 
                             mode='min', 
                             verbose=1)
...
history_mobilenet = train_model(model_mobilenet, train_generator, valid_generator, epochs=50, callbacks=[checkpoint])
...
best_weights_path = "MobileNet_checkpoint_best_"+str(formatted_result)+".h5"           
