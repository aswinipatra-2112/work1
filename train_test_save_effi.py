...
def build_mobilenet_model(input_shape):
    ...
    base_model = EfficientNetB0(weights = weights_path, include_top=False, input_shape=input_shape)
    x = base_model.output
...
checkpoint = ModelCheckpoint(f"Effi_checkpoint_best_{{val_loss:.4f}}.h5", 
                             monitor='val_loss', 
                             save_best_only=True, 
                             mode='min', 
                             verbose=1)
...
best_weights_path = "Effi_checkpoint_best_"+str(formatted_result)+".h5"           
