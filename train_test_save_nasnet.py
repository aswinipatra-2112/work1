...
def build_mobilenet_model(input_shape):
    weights_path = 'NASNet-mobile-no-top.h5'
    base_model = nasnet.NASNetMobile(weights = weights_path, include_top=False, input_shape=input_shape)
    x = base_model.output
...
checkpoint = ModelCheckpoint(f"NASNetMobile_checkpoint_best_{{val_loss:.4f}}.h5", 
                             monitor='val_loss', 
                             save_best_only=True, 
                             mode='min', 
                             verbose=1)
...
history_mobilenet = train_model(model_mobilenet, train_generator, valid_generator, epochs=30, callbacks=[checkpoint])
...
best_weights_path = "NASNetMobile_checkpoint_best_"+str(formatted_result)+".h5"           
...
image_name="NASNetMobile_loss_acc"
